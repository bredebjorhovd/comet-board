//! §gh#55: edge-less (local) mode. `COMET_EDGE_URL=off` is a stated
//! configuration, not graceful failure: no edge transport may be built — no
//! room joins, no peer link cache / device room, no release poller — and auth
//! is forced to dev mode even when a WorkOS client id (or dev bearer) is
//! configured.
...
fn local_config(data_dir: &std::path::Path) -> EngineConfig {
    EngineConfig {
        ...
        // The board polls trackers over its own HTTP, not the edge — §gh#55 is
...
async fn edge_off_builds_no_edge_transports() {
    ...
    // Opening a chat must not spawn a room join (the pre-§gh#55 behavior
    // logged a "session room join failed" warning per open); the handle stays
    // local.
