...
mod tests {
    ...
    /// The reason this module exists, measured: one 1.6MB reply streamed on
    /// the 120ms commit cadence used to re-send the whole transcript per tick.
    #[test]
    fn streamed_reply_delta_payload_is_a_fraction_of_whole_transcript() {
        let prior = entry("m-user", "the prompt");
        let chunk = "x".repeat(8 * 1024);
        let mut reply = String::new();
        let mut delta_bytes = 0usize;
        let mut whole_bytes = 0usize;
        let mut prev = vec![prior.clone()];
        for _ in 0..200 {
            reply.push_str(&chunk);
            let next = vec![prior.clone(), entry("m-reply", &reply)];
            let frame = diff_transcript(&prev, &next);
            delta_bytes += serde_json::to_vec(&frame).unwrap().len();
            whole_bytes += serde_json::to_vec(&next).unwrap().len();
            let mut applied = prev.clone();
            apply_transcript_frame(&mut applied, frame).unwrap();
            assert_eq!(applied, next);
            prev = next;
        }
        eprintln!(
            "streamed 1.6MB reply over 200 ticks: delta wire bytes {delta_bytes}, \
             whole-transcript wire bytes {whole_bytes} ({}x)",
            whole_bytes / delta_bytes.max(1)
        );
        assert!(
            delta_bytes * 50 < whole_bytes,
            "delta protocol regressed: {delta_bytes} vs {whole_bytes}"
        );
    }
