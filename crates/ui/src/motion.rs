...
/// — on the board, a row per running agent made display-rate repaints the
/// steady-state cost of an idle-looking window (gh#415). A shared 30fps clock
/// is visually equivalent for these chunky cell waves at a quarter of the
/// redraws, and a window with no spinner mounted schedules nothing at all.
...
// A lease must survive several ticks, or a briefly-throttled view would fall
// off the list while its spinner is still mounted.
const _: () = assert!(PULSE_LEASE.as_millis() >= 4 * PULSE_TICK.as_millis());
...
// The loader curves are shared with the phone (`comet_proto::motion`; iOS
// mirrors the constants in `Theme/Motion.swift`): a loading indicator is a
// brand surface, and two of them that disagree read as two products. The
// constants and the math live there; this crate animates them with gpui.
