...
//!
//! Rendering pattern: every cell reads its phase off [`motion::pulse_delta`] —
//! one shared wall-clock for all repeating loaders, ticking the mounted views
//! at ~30fps instead of each cell requesting display-rate frames (gh#415).
//! Per-cell offsets come from [`motion::staggered_phase`], and one epoch means
//! all instances stay phase-locked. Cells animate inside fixed-size slots —
//! opacity and inner size are paint-local and never move surrounding layout.
//! Reduced motion renders the rest state and schedules nothing.
...
/// follows the 820:940 canvas). `view` is the entity to keep ticking while the
/// loader is mounted — pass `cx.entity_id()` from the rendering view.
...
pub fn mini_gradient_spinner(cell_px: f32, view: EntityId, cx: &mut App) -> impl IntoElement {
    const COLS: usize = 2;
    const ROWS: usize = 3;
...
pub fn splash_overlay(theme: &Theme, fading: bool, view: EntityId, cx: &mut App) -> AnyElement {
    let content = div()
        ...
        .justify_center()
        .gap(px(28.0))
        .child(comet_mark_loader(theme, 64.0, view, cx))
