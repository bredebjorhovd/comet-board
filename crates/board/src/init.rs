...
use crate::config::{GithubAuth, Paths, github_auth, shorten_home};
use anyhow::Result;
    ...
    F: Fn(&str) -> Option<SpaceRepo>,
{
    ...
    // Resolve every git-detected space once; the routes below read from this.
    ...
    let body = starter_config(&routes, &repo_list);
...
fn starter_config(routes: &str, repo_list: &str) -> String {
    format!(
...
mod tests {
    ...
    fn the_generated_starter_config_parses() {
        ...
        assert!(
            !body.to_lowercase().contains("linear"),
            "the starter config must not offer Linear (gh#471)"
        );
    }
