...
//!
//! ```sh
//! cargo test -p comet-board ios_review_spec      # this half, and the guard
...
fn build() -> Value {
    ...
    json!({
        "note": "Generated by crates/board/tests/ios_review_spec.rs. \
                 Do not hand-edit — run `UPDATE_REVIEW_SPEC=1 cargo test -p comet-board ios_review_spec`, \
...
fn the_cross_language_fixture_matches_this_reading() {
    ...
    let on_disk = std::fs::read_to_string(&path).unwrap_or_else(|err| {
        panic!(
            "cannot read {}: {err}. Run `UPDATE_REVIEW_SPEC=1 cargo test -p comet-board ios_review_spec`",
            path.display()
        )
    });
    ...
    panic!(
        "the review reading has moved and apps/ios/Comet/Spec/review-spec.json has not.\n{}\n\n\
         Regenerate with `UPDATE_REVIEW_SPEC=1 cargo test -p comet-board ios_review_spec`, then \
