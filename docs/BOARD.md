...
- **`spaceTitles` glued the qualifier back on.** The phone still returned
  `"{base} · {tail}"` as one string — exactly what §H33 fixed on the desktop —
...
- **The orchestrator's kill switch was unreachable.** §H33 gave the slot a menu
...
- **`comet-board member add <email> --github <login|address>`.** §H22 gave
  ...
  `GIT_AUTHOR_EMAIL` and produce exactly the unattributable commits §H22
...
- **The resolution happens on the box, for §H17's reason.** A bare login needs
...
- **`member list` is the pairing nobody thinks about.** A mapped teammate with
  ...
  and "the engine could not be asked" stay different answers (§H36).
...
- **A test that was reading the live board.** `Paths::under` honours
  ...
  two files that were looked at — see §H47.)
...
### H46 — A release is not done until the edge says so — **done** (gh#197)
...
been actively harmful — the upgrade existed to ship §H37's `comet-board`, and the
...
### H47 — A test's board is the directory it made — **done** (gh#190)
...
`cargo test`. §H40 found this failure and fixed the two files it was looking at.
...
- **The variables are an answer, not an instruction.** `COMET_BOARD_CONFIG_DIR` /
  ...
  as `GIT_ASKPASS` with none of its parent's arguments (§H11). The engine exports
...
- **Every hand-built `Paths` went back through the constructor.** The four
  helpers §H40 left behind (`device_routing.rs`, `board_members.rs`,
  ...
  is the actual fix, since the next test to be written will not have read §H40.
...
- **Noted, not fixed:** `Repos::new` resolves its worktree root from
  ...
  (§H45's), and the fix is a seam in `Repos`, not in the board's paths.
