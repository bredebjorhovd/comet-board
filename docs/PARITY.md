...
| Item | Status | Notes |
| --- | --- | --- |
| 1.1 Window shell | partial | gpui window, always-dark by default with an opt-in light theme (Settings → Appearance, `COMET_THEME` env, `ui-settings.json`), external links via OS browser. Deferred: frameless-inset/traffic-light chrome (macOS packaging not executed), single-instance lock, dev-vs-packaged port split (env vars instead). |
...
| 1.7 Composer | done (+addition) | Progressive disclosure for skills (gh#134): `/` at the start or after whitespace opens an inline picker of what the RUN can invoke (`ListSkills`, slot-aware), filtering as you type, Enter/Tab completing, Escape closing without editing the text; same on the TUI. Otherwise: Send/Steer/Stop morph, compact↔expanded flip, per-chat drafts that outlive the app (gh#536 — `{data_dir}/composer-drafts.json`, text + caret, device-local and never synced; keyed per chat and per space's new-chat canvas; cleared only by sending or by emptying the box), optimistic echo with failure return-to-draft (into the chat it was written for, not into whatever is on screen when the failure lands), QuestionPanel (paged, auto-advance, number keys), all four pickers (harness/model, traits, repo with folder browser + clone/create, branch with worktree toggle), image attachments (paste/drop/picker → strip → chunked upload to host device → `withAttachments` refs in prompt text + inline image blocks for the Claude harness; per-chat stash, failure hand-back, lightbox — `ui/src/attachments.rs`). |
| 1.8 Transcript | done (+addition) | Skill invocations are landmarks, not tool rows (gh#134): a `Skill` tool call (and a `<command-name>` block on the stream) decodes to `ToolCall::Skill{name,args}`, breaks the tool group around it, and renders as an accent-tinted block — name prominent, arguments secondary — on all three surfaces. Otherwise: doc-projection source, virtualized, markdown + syntax highlight, tool folding (ToolGroup/ToolChip), input/error chips, stick-to-bottom band, MessageRail minimap (hover preview, hidden < 48rem), user-bubble attachment thumbnails (112×80, read-back from owning device, 2s→15s retry ladder, seeded cache, click-to-expand lightbox). |
...
| ListSkills | done (addition) | Not in comet: what a run in this chat could invoke, for the composer's `/` picker (gh#134). Forwardable, and it has to be — skills are files on the chat's host, and a chat naming an agent account reads that slot's config dir rather than the box user's. |
