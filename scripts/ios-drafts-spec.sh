#!/usr/bin/env bash
# The phone's composer drafts, asserted in the simulator (gh#536).
#
# `apps/ios/Comet/Composer/DraftStore.swift` is where a half-written prompt
# lives now that it is no longer a `@State` string dying with its view. This
# builds the app and runs `DraftsSpecRunner` (`-drafts-spec`) against it: a
# dictionary and a file, no network, no session, no edge.
#
# The desktop half of the same rules needs no simulator and runs in CI:
#
#   cargo test -p comet-ui drafts            the store, and the composer's
#                                            navigation/quit/clear behaviour
#   scripts/ios-drafts-spec.sh               this: the phone's copy of it
#
# Usage: scripts/ios-drafts-spec.sh [simulator name]
...
DERIVED="$(mktemp -d /tmp/comet-ios-drafts-spec.XXXXXX)"

cleanup() { rm -rf "$DERIVED"; }
trap cleanup EXIT

echo "drafts spec: building for $SIM"
...
echo "drafts spec: booting $SIM ($APP_ID)"
...
LOG="$(xcrun simctl get_app_container "$SIM" "$APP_ID" data)/Documents/drafts-spec.log"
rm -f "$LOG"
xcrun simctl launch "$SIM" "$APP_ID" -drafts-spec >/dev/null
