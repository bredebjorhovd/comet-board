#!/usr/bin/env bash
# The Swift half of the cross-language review fixture (gh#256).
#
# `apps/ios/Comet/Board/ReviewModels.swift` is a second implementation of the
# reading in `comet_board::claims` and `comet_board::effects` — no Rust runs on
# that device — and two implementations of one rule is how a phone comes to
# disagree with a laptop about whether an attempt is trustworthy. The review
...
#
# Usage: scripts/ios-review-spec.sh [simulator name]
...
DERIVED="$(mktemp -d /tmp/comet-ios-review-spec.XXXXXX)"

cleanup() { rm -rf "$DERIVED"; }
trap cleanup EXIT

echo "review-spec: building for $SIM"
...
echo "review-spec: booting $SIM ($APP_ID)"
...
LOG="$(xcrun simctl get_app_container "$SIM" "$APP_ID" data)/Documents/review-spec.log"
rm -f "$LOG"
xcrun simctl launch "$SIM" "$APP_ID" -review-spec >/dev/null

# The runner decodes a bundled file and runs pure functions over it — no
# network, no session — so it is done long before this poll gives up.
