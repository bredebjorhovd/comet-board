...
| Sidebar chat menu: send / stop the board's notices (gh#104, gh#144, gh#348) | Same item, same words, on both surfaces (gh#166): the chat screen's ⋯ menu, a long-press on its Active or Needs-you row, and a long-press on the notices slot itself. Setting it asks first and names what it replaces — one `fallback_chat` key, so a second chat moves it; stopping is immediate, and is the phone's only route to `routes defaults fallback_chat --unset`. Never offered on a chat the board dispatched |
