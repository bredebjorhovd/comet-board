...
actor DeviceRelayClient {
    ...
    /// attachment uploads pass longer ones — first chunk 90s for a cold dial to
    /// a device that may be the box, commit 150s to outlast the assemble and
    /// its edge mirror (the desktop's numbers, `crates/ui/src/attachments.rs`).
